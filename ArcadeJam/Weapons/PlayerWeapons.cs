...
public abstract class PlayerWeapon {
    ...
    protected void FireAtAngle(float angle, float speed, Vector2 startPos) {
        angle *= (float)(Math.PI / 180); //converting to radians
        float y = -(float)(Math.Cos(angle) * speed);
        float x = (float)(Math.Sin(angle) * speed);
        NodeManager.AddNode(new PlayerBullet(new Vector2Data(x, y), startPos));

    }
}


public class Level1 : PlayerWeapon {
    public Level1(FloatRect pos, BoolData focused) : base(pos, focused, 0f, 300) {
    }
